...
from Configurations.fd1t_cfg_2d import mod, modMax, modMin, modSig, modTrans, sim, simMax, simMin, idxModTypes, idxSimTypes
    ...
    for frame in range(numFrames):
        ...
        for particleId in range(numParticles):
            ...
            for step in range(numSteps):
                ...
                if posx < leftBound or posx > rightBound or posy < leftBound or posy > rightBound:
                ...
                for _ in range(photons):
                    ...
                    locx = posx + photonSig * randn(rng, seqId)
                    locy = posy + photonSig * randn(rng, seqId)
...
class DataGeneratorFD1T(Sequence):
    def __init__(self, batches, batchSize, maxepochs,
        ...
        self.position = np.empty((self.numSeq, self.maxParticles, 2), dtype='float32')  # 2D
...
dg_train_2d = DataGeneratorFD1T(batches=CNN.BATCHES_PER_PARTITION, batchSize=CNN.BATCH_SIZE, reachfullatepoch=CNN.REACH_FULL_RANGE,
