...
# NOTE: We divide EmissionRate by StepsPerFrame, i.e. 10, and ParticleSig by sqrt(StepsPerFrame), i.e. sqrt(10), in the data generation code. See fd1t_3d_datagen.py.
# base values of model parameters
mod = [0] * idxModTypes
mod[cfg.EnumModelParams.EmissionRate.value]        =  11.22      # average count rate per particle per frame
mod[cfg.EnumModelParams.ParticleDensity.value]     =  0.48374    # avg number of particles per pixel
mod[cfg.EnumModelParams.ParticleSig.value]         =  0.15969    # std of particle [pixel per frame]
mod[cfg.EnumModelParams.PhotonSig.value]           =  0.708125
...
# See randomParModel function in simulator > fd1t_3d_datagen.py
...
modSig[cfg.EnumModelParams.EmissionRate.value]     =  9.18        # e.g. CPS 1k to 10k, frame time 2.04 ms
...
modSig[cfg.EnumModelParams.PhotonSig.value]        =  0.03219     # e.g. PSF 1.05 to 1.15, with mean of 1.10, with WL 515 nm and NA 1.0, magnification 60X. It's uniformly sampled.
...
sim[cfg.EnumSimParams.NoiseType.value]      =  cfg.EnumNoiseType.Gaussian.value  # type of camera noise during simulation: .Gaussian, .Experimental_EMCCD, .MixNoise
...
# Additional params for 3D simulations
Sigma0 = 1.1
SigmaZ = 2.2
NA = 1  # numerical aperture of the objective. Please make sure that zfac is valid.
Wavelength = 515e-9 
PixelSizeRS = 24e-6  # size of camera pixel
Mag = 60  # objective magnification
PixelSize = PixelSizeRS / Mag

sim[cfg.EnumSimParams.ZDimFactor.value]             = 10.0  # factor multiplied to LightSheetThickness: range of z-dim for the particles to move in 3D simulation
sim[cfg.EnumSimParams.ZFac.value]                   = math.tan(math.asin(NA/1.333))
sim[cfg.EnumSimParams.LightSheetThickness.value]    = SigmaZ * Wavelength / NA / 2 / PixelSize
